                ...
                <div class="card card-custom">
                    <div class="card-header">
                        <div class="card-title">
                            ...
                            <h3 class="card-label">{{admin('Store Features')}}</h3>
                        </div>
                        <div class="card-toolbar">
                            ...
                            <!--end::Dropdown-->
                            <!--begin::Button-->
                            <a href="{{route('admin.settings.store_features.create')}}"
...
@section('scripts')
        ...
        var KTDatatablesExtensionButtons = function () {
            ...
            var initTable1 = function () {
                ...
                table.DataTable({
                    ...
                    "ajax": {
                        url: "{{route('admin.settings.store_features.load')}}",
                        data: function (d) {
                    ...
                    columnDefs: [
                        {
                            ...
                            render: function (data, type, full, meta) {
                                var x = '<a  href="/admintz/settings/store_features/edit/' + full.id + '" type="submit" style="margin:1px;" class="btn btn-sm btn-icon btn-icon-md btn-success btn-elevate btn-pill btn-elevate-air la la-edit fa-lg" id="edit_btn" title="{{admin('Edit')}}"></a>';
                                {{--x = x + '<a type="submit" href="/admintz/settings/store_features/details/' + full.id + '"style="margin:1px;" class="btn btn-sm btn-icon btn-icon-md btn-info btn-elevate btn-pill btn-elevate-air la la-eye fa-lg" id="details_btn" title="{{admin('View')}}"></a>';--}}
    ...
    <script>
        ...
        function removeTableRow(id, swal) {
            var table = $('#kt_datatable').DataTable();
            $.ajax({
                url: "/admintz/settings/store_features/delete",
                type: "post",
                data: {
                    id: id,
                    _token: '{{csrf_token()}}'
        ...
        function disableTableRow(id, swal) {
            var table = $('#kt_datatable').DataTable();
            $.ajax({
                url: "/admintz/settings/store_features/disable",
                type: "post",
                data: {
                    id: id,
                    _token: '{{csrf_token()}}'
        ...
        function activateTableRow(id, swal) {
            var table = $('#kt_datatable').DataTable();
            $.ajax({
                url: "/admintz/settings/store_features/activate",
                type: "post",
                data: {
                    id: id,
                    _token: '{{csrf_token()}}'
