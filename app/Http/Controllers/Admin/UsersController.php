...
class UsersController extends Controller
{
    ...
    {
        $this->middleware('can:user-edit', ['only' => ['edit','update','activate','disable']]);
        $this->middleware('can:user-view', ['only' => ['index','load']]);
    ...
    {
        $items = User::query()->where('is_admin',0);
    ...
    {
        return view('admin.pages.users.storeOwners.index');
    }

    public function loadStoreOwners(Request $request)
    {
        $items = User::query()->where('is_admin',1)->where('role_id',3);
        $search = $request->get('search', false);
        if ($search) {
            $items = $items->where(function ($query) use ($search) {
                $query->where('first_name', 'like', '%' . $search . '%')
        ...
        return DataTables::make($items)
            ...
            ->addColumn('store_name', function ($item) {
                return Store::query()->where('user_id',$item->id)->first()->store_name;
            })
            ->addColumn('store_id', function ($item) {
                return Store::query()->where('user_id',$item->id)->first()->id;
            })
            ->addColumn('country_name', function ($item) {
                return $item->country->_name;
            })
